 ...
 * @type {string[]}
 */
export const nonFlagArgs = args.slice(1).filter((arg) => !arg.startsWith("-"));
...
export async function corruptCommand() {
     ...
     * @default 0.5
     */
    const deepfryContrast = Number(args.find((arg) => arg.startsWith("-dc=") || arg.startsWith("--df-contrast="))?.split("=")[1] ?? 0.5);
    /**
     ...
     * @default 10
     */
    const deepfryQualityDamage = Number(args.find((arg) => arg.startsWith("-dqd=") || arg.startsWith("--df-qual-dmg="))?.split("=")[1] ?? 10);
    /**
     ...
     * @default 0.25
     */
    const deepfryDamagingQuality = Number(args.find((arg) => arg.startsWith("-dq=") || arg.startsWith("--df-qual="))?.split("=")[1] ?? 0.25);
    /**
     * The mode to use.
     *
     * Options:
        ...
        if (mode === "deepfry") {
            ...
            for (let x = 0; x < srcImg.width; x += scaleX) {
                for (let y = 0; y < srcImg.height; y += scaleY) {
                     ...
                     * @type {typeof mode}
                     */
                    const newMode = mode === "random" ? nonRandomModes.filter((m) => m !== "deepfry")[Math.floor(Math.random() * (nonRandomModes.length - 1))] : mode;
                    switch (newMode) {
                        case "randomColor": {
