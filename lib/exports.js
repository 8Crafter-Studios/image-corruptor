 ...
 */
export const format_version = "1.0.0-alpha.4";
/**
...
export async function corruptImage(src, options = {}) {
    ...
    for (let x = 0; x < srcImg.width; x++) {
        for (let y = 0; y < srcImg.height; y++) {
             ...
             */
            const newMode = options.mode === "random" ? nonRandomModes[Math.floor(Math.random() * nonRandomModes.length)] : options.mode ?? "randomColor";
            switch (newMode) {
                case "randomColor": {
